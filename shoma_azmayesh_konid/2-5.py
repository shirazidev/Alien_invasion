...
class Alien(pygame.sprite.Sprite):
    ...
    def __init__(self, ai_game, column_number):
        ...
        self.rect = self.image.get_rect()
        self.rect.x = ai_game.settings.screen_width - (column_number * self.rect.width)
        self.rect.y = random.randint(0, ai_game.settings.screen_height - self.rect.height)
        self.x = float(self.rect.x)

    def update(self, ai_game):
        """Move the alien toward the right."""
        self.x -= self.settings.alien_speed
        self.rect.x = self.x

        if pygame.sprite.spritecollideany(self, ai_game.bullets):
            ai_game.bullets.remove(self)
            ai_game.aliens.remove(self)
...
class AlienInvasion:
    ...
    def _update_bullets(self):
        ...
        if not self.aliens:
            # No more aliens left, create a new fleet
            self._create_fleet()

    def _create_fleet(self):
        """Create the fleet of aliens."""
        alien_height = Alien(self, 0).rect.height
        available_space_y = self.settings.screen_height - (2 * alien_height)
        number_of_rows = available_space_y // (2 * alien_height)
        for row_number in range(number_of_rows):
            for column_number in range(self.settings.aliens_per_column):
    ...
    def _create_alien(self, column_number, row_number):
        ...
        alien = Alien(self, column_number)
